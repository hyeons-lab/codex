...
const CLI_AUTH_KEYRING_SERVICE: &str = "codex";
...
impl AuthStorageBackend for DirectKeyringAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        ...
        match self
            .keyring_store
            .load(CLI_AUTH_KEYRING_SERVICE, &account)
    ...
    fn save(&self, auth: &AuthDotJson) -> std::io::Result<()> {
        ...
        self.keyring_store
            .save(CLI_AUTH_KEYRING_SERVICE, &account, &serialized)
            .map_err(|err| std::io::Error::other(err.message()))?;
    ...
    fn delete(&self) -> std::io::Result<bool> {
        ...
        let keyring_removed = self
            .keyring_store
            .delete(CLI_AUTH_KEYRING_SERVICE, &account)
            .map_err(|err| std::io::Error::other(err.message()))?;
...
pub(super) fn create_auth_storage(
    ...
    create_auth_storage_with_keyring_backend_kind(
        codex_home,
        mode,
...
pub(super) fn create_auth_storage_with_keyring_backend_kind(
    codex_home: PathBuf,
    mode: AuthCredentialsStoreMode,
    keyring_backend_kind: CliAuthKeyringBackendKind,
) -> Arc<dyn AuthStorageBackend> {
    let keyring_store: Arc<dyn KeyringStore> = Arc::new(DefaultKeyringStore);
    create_auth_storage_with_keyring_store(codex_home, mode, keyring_store, keyring_backend_kind)
}
